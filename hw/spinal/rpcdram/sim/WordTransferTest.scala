 ...
 */
object WordTransferTest extends RpcDramPhyTestBase("WordTransferTest", RpcDramConfig(simMode = true, freqMHz = 800)) with App {
  runTest()
  ...
  override protected def runSpecificTest(dut: RpcDramPhy): Unit = {
      ...
      while (true) {
        ...
        if (transferActive) {
          ...
          if (cycleCount >= 8) {
            ...
            println(s"[TRANSFER] Total bandwidth: ${256 * 800 / 8} Mbps per WORD")
          }
        }
      }
    }
    ...
    println(f"✓ Peak bandwidth capability: 3.2GB/sec at 800MHz")
  }
}