...
// =============================================================================
// Memory State Formal Verification
// =============================================================================

// class DutWithBankMemory(cfg: RpcDramConfig) extends Component {
//     // Simplified memory model for formal verification
//     val bankActive = Vec(Reg(Bool()) init(False), cfg.bankCount)
//     val openRow = Vec(Reg(UInt(cfg.rowAddrWidth bits)) init(0), cfg.bankCount)
//
//     val activate = slave Stream(new Bundle {
//       val bank = UInt(cfg.bankAddrWidth bits)
//       val row = UInt(cfg.rowAddrWidth bits)
//     })
//
//     val precharge = slave Stream(UInt(cfg.bankAddrWidth bits))
//
//     when(activate.fire) {
//       bankActive(activate.payload.bank) := True
//       openRow(activate.payload.bank) := activate.payload.row
//     }
//
//     when(precharge.fire) {
//       bankActive(precharge.payload) := False
//       openRow(precharge.payload) := U(0, cfg.rowAddrWidth bits)
//     }
//
//     activate.ready := True
//     precharge.ready := True
//
//     // Skip complex assertions for now - focus on coverage
//   })

// object MemoryStateFormal extends App {
//   FormalConfig.withBMC(10).doVerify(new Component {
//     val cfg = RpcDramConfig(simMode = true)
//     val dut = FormalDut(new DutWithBankMemory(cfg))
//
//     assumeInitial(ClockDomain.current.isResetActive)
//
//     // Drive with random but valid commands
//     anyseq(dut.activate.valid)
//     anyseq(dut.activate.payload.bank)
//     anyseq(dut.activate.payload.row)
//     anyseq(dut.precharge.valid)
//     anyseq(dut.precharge.payload)
//
//     // Bank signals are already constrained by their width
//
//     // No simultaneous activate and precharge to same bank
//     when(dut.activate.fire && dut.precharge.fire) {
//       assume(dut.activate.payload.bank =/= dut.precharge.payload)
//     }
//
//     // Properties to verify
//     val anyBankActive = dut.bankActive.reduce(_ || _)
//     val allBanksInactive = !anyBankActive
//
//     // Cover interesting states
//     cover(anyBankActive)
//     cover(allBanksInactive)
//     cover(dut.bankActive.reduce(_ && _)) // All banks active
//
//     // Verify we can activate specific rows
//     val targetBank = anyconst(UInt(cfg.bankAddrWidth bits))
//     val targetRow = anyconst(UInt(cfg.rowAddrWidth bits))
//     // Constraints are implicit due to bit width
//
//     cover(dut.bankActive(targetBank) && dut.openRow(targetBank) === targetRow)
//   })
// }
