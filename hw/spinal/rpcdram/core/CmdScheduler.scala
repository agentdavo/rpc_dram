...
case class CmdScheduler(cfg: RpcDramConfig) extends Component {
  ...
  val cmdValidationArea = new Area {
    ...
    val lastValidatedCmd = Reg(DramCmd(cfg))
    lastValidatedCmd.isSerial init(False)
    lastValidatedCmd.opcode init(Opcodes.PAR_NOP)
    lastValidatedCmd.bank init(0)
    lastValidatedCmd.rowAddr init(0)
    lastValidatedCmd.colAddr init(0)
    lastValidatedCmd.burstCount init(0)
    lastValidatedCmd.writeMask init(0)
    lastValidatedCmd.odt init(False)
  }
  ...
  def validateCommandTransition(currentCmd: DramCmd, nextCmd: DramCmd): Bool = {
    ...
    when(currentOpcode === Opcodes.PAR_NOP) {
      return True // NOP can transition to any command
    }.elsewhen(nextOpcode === Opcodes.PAR_NOP || nextOpcode === Opcodes.PAR_RESET) {
      return True // NOP and RESET can be issued anytime
    }.elsewhen(nextOpcode === Opcodes.PAR_ZQ) {
      return True // ZQ commands can be issued after MRS or anytime during init
    }.elsewhen(nextOpcode === Opcodes.PAR_REF) {
      return True // REF commands can generally be issued anytime
    }

    // Use conditional logic for table selection (SpinalHDL doesn't support pattern matching with Bool)
    val legal = Bool()

    when(!currentIsSerial && nextIsSerial && sameBank) {
  ...
  val fsm = new StateMachine {
    ...
    val pendingCmd = Reg(DramCmd(cfg))
    pendingCmd.isSerial init(False)
    pendingCmd.opcode init(Opcodes.PAR_NOP)
    pendingCmd.rank init(0)
    pendingCmd.bank init(0)
    pendingCmd.rowAddr init(0)
    pendingCmd.colAddr init(0)
    pendingCmd.burstCount init(0)
    pendingCmd.writeMask init(0)
    ...
    commandPhase.whenIsActive {
      ...
      switch(phaseSubState) {
        is(1) { // Validate substate
          ...
          cmdValidationArea.isLegal := cmdLegal
